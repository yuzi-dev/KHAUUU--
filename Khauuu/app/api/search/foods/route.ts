...
export async function GET(request: NextRequest) {
  try {
    ...
    const vegetarianOnly = searchParams.get('vegetarian_only') === 'true'
    ...
    // Use the search_foods function from the database
    const { data: foods, error } = await supabaseServer
      .rpc('search_foods', {
        search_query: query.trim(),
        limit_count: limit,
        offset_count: offset,
        restaurant_filter: restaurantId || null,
        category_filter: category || null,
        vegetarian_filter: vegetarianOnly
      })
